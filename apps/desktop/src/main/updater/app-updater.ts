...
// electron-updater is CommonJS and defines its exports with lazy getters, which
// Node's ESM loader cannot detect statically: `import { autoUpdater }` throws at
// load time. The default import is the whole module namespace. Types are erased
// before runtime, so those can stay named. Of every external this bundle keeps,
// electron-updater is the only one that needs this.
...
// named export since v5: the default export was removed
      ...
      .then(async (returnValue) => {
        if (returnValue.response === 0) {
          ...
          return (
            autoUpdater
              .downloadUpdate()
              // downloadUpdate resolves with the downloaded files, but the caller
              // reads this as "was the update ignored" - starting a download is not
              .then(() => false)
              .catch(async (err) => {
