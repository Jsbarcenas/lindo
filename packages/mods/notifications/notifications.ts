    ...
    if (!this.wGame.document.hasFocus() && this.rootStore.optionStore.gameNotification.itemSold) {
      if (e.msgId === 65) {
        ...
        if (this._ressourcesKnow[id] == null) {
          const { dofusOrigin, buildVersion } = this.rootStore.appStore
          const url = `${dofusOrigin.replace(/\/$/, '')}/data/map?lang=${
            this.rootStore.appStore.language
          }&v=${buildVersion}`
