...
import { DOFUS_ORIGIN } from '../../constants'
...
export const AppStoreModel = types
  ...
  .props({
    ...
    _language: types.maybe(types.enumeration(LANGUAGE_KEYS)),
    dofusTouchEarly: types.optional(types.boolean, false),
    /** the origin the game was actually served from, resolved by the game updater at startup */
    dofusOrigin: types.optional(types.string, DOFUS_ORIGIN)
  })
  .views((self) => ({
  ...
  .actions((self) => ({
    ...
    setDofusOrigin(dofusOrigin: string) {
      if (self.dofusOrigin !== dofusOrigin) {
        self.dofusOrigin = dofusOrigin
      }
    }
  }))
