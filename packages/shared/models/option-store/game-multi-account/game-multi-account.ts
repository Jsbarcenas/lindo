...
import type { RootStoreEnv } from '../../root-store'
...
export const GameMultiAccountModel = types
  ...
  .actions((self) => ({
    ...
    addCharacter: flow(function* (character: GameCharacterSnapshotIn) {
      const { encryptCharacterPassword } = getEnv<RootStoreEnv>(self)
      if (!encryptCharacterPassword) {
        throw new Error('addCharacter is only available in the renderer process')
      }
      self.characters.push({
        ...character,
        password: yield encryptCharacterPassword(character.password)
      })
    }),
