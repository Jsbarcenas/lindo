import { Game, RootStore } from '@lindo/client-store'
import { Mod, MODS, NotificationsMod } from '@lindo/mods'
import { DofusWindow } from '@lindo/dofus-window'
...
export const useGameManager = ({ game, rootStore, LL }: GameManagerProps) => {
  ...
  useEffect(() => {
    // an MST map is an ObservableMap at runtime, but IMSTMap does not declare it as
    // one, so the map overload of observe has to be selected explicitly. The change
    // key is the model identifier, which is what this used to read off MST's
    // internal node through the untyped oldValue.
    const games = rootStore.gameStore._games as unknown as ObservableMap<string, unknown>
    return observe(games, (change) => {
      if (change.type === 'delete' && change.name === gameId) {
        destroyMods()
      }
    })
  }, [])
    ...
    init: (dWindow: DofusWindow) => {
      ...
      const startMods = () => {
        for (const key in MODS) {
          // One mod must not be able to take the game down with it. These bind
          // to the Dofus client's internals, so an upstream rename throws here;
          // unguarded, that aborted the whole handler and left a black screen
          // with the game running behind it.
          try {
            ...
            window.lindoAPI.logger.error(`failed to start mod "${key}"`, error)()
          }
        }
      }
