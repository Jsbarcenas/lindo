...
import { DOFUS_EARLY_ORIGIN, DOFUS_ORIGIN, LANGUAGES, RESOLUTIONS } from '@lindo/shared'
import { useStores } from '@lindo/client-store'
      ...
      {() => (
        <>
          <Box sx={{ p: 2, flexGrow: 1, flex: 1 }}>
            ...
            {/*
              The checkbox is a preference, not a fact: when the preferred origin
              cannot be reached the updater silently falls back to the other one,
              which used to leave the app running against a different environment
              than the box implied. Show what is actually in use.
            */}
            <Typography variant='caption' color='text.secondary'>
              {appStore.dofusOrigin}
              {appStore.dofusOrigin !== (appStore.dofusTouchEarly ? DOFUS_EARLY_ORIGIN : DOFUS_ORIGIN) &&
                ' — the origin above could not be reached, so this one is in use'}
            </Typography>
          </Box>
        </>
      )}
